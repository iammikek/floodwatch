...
>
    <div class="max-w-2xl mx-auto w-full">
        ...
        <div wire:loading wire:target="search" class="w-full flex flex-row flex-nowrap items-center gap-3 p-4 rounded-lg bg-white dark:bg-slate-800 shadow-sm border border-slate-200 dark:border-slate-700">
            ...
            <span class="text-slate-600 dark:text-slate-400 text-sm flex-1 min-w-0 inline-block" wire:stream.replace="searchStatus">{{ __('flood-watch.dashboard.connecting') }}</span>
        </div>
        ...
        @if (!$loading && $assistantResponse)
            ...
            >
                <div class="flex flex-wrap items-center justify-between gap-3">
                    ...
                    >
                        <label class="inline-flex items-center gap-2 cursor-pointer">
                            ...
                            />
                            <span class="text-sm text-slate-600 dark:text-slate-400">{{ __('flood-watch.dashboard.auto_refresh') }}</span>
                        </label>
                        <span
                        ...
                        >
                            ...
                            @if (count($incidents) > 0)
                                <div class="px-3 py-2 bg-blue-50/50 dark:bg-blue-900/10 border-t border-slate-200 dark:border-slate-700">
                                    <p class="text-xs font-medium text-blue-800 dark:text-blue-300 mb-1.5">{{ __('flood-watch.dashboard.road_incidents_on_map') }}</p>
                ...
                <div id="flood-risk">
                    <h2 class="text-lg font-medium text-slate-900 dark:text-white mb-3">{{ __('flood-watch.dashboard.flood_warnings') }}</h2>
                    @if (count($floods) > 0)
                        <ul class="space-y-3">
                            @foreach ($floods as $flood)
                                <li class="p-4 rounded-lg bg-white dark:bg-slate-800 shadow-sm border border-slate-200 dark:border-slate-700 text-left overflow-visible">
                                    <p class="font-medium text-slate-900 dark:text-white">{{ $flood['description'] ?? __('flood-watch.dashboard.flood_area') }}</p>
                                    <p class="text-sm text-amber-600 dark:text-amber-400 mt-1">{{ $flood['severity'] ?? '' }}</p>
                                    @if (!empty($flood['distanceKm']) && $hasUserLocation)
                                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">{{ __('flood-watch.dashboard.km_from_location', ['distance' => $flood['distanceKm']]) }}</p>
                                    @endif
                                    ...
                                    @if (!empty($flood['message']))
                                        <div x-data="{ open: false }" class="mt-2">
                                            <button type="button" @click="open = !open" class="flex items-center gap-2 cursor-pointer text-amber-600 dark:text-amber-400 hover:text-amber-700 dark:hover:text-amber-300" aria-label="{{ __('flood-watch.dashboard.toggle_message') }}">
                    ...
                    @else
                        <p class="p-4 rounded-lg bg-white dark:bg-slate-800 shadow-sm border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400">{{ __('flood-watch.dashboard.no_flood_warnings') }}</p>
                    @endif
                </div>

                <div id="forecast">
                    <h2 class="text-lg font-medium text-slate-900 dark:text-white mb-3">{{ __('flood-watch.dashboard.forecast_outlook') }}</h2>
                    @if (count($forecast) > 0 && !empty($forecast['england_forecast']))
                        <div class="p-4 rounded-lg bg-white dark:bg-slate-800 shadow-sm border border-slate-200 dark:border-slate-700">
                            ...
                            @if (!empty($forecast['issued_at']))
                                <p class="text-xs text-slate-400 dark:text-slate-500 mt-1">{{ __('flood-watch.dashboard.issued') }}: {{ \Carbon\Carbon::parse($forecast['issued_at'])->format('j M Y, g:i') }}</p>
                            @endif
                        </div>
                    @else
                        <p class="p-4 rounded-lg bg-white dark:bg-slate-800 shadow-sm border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400">{{ __('flood-watch.dashboard.no_forecast') }}</p>
                    @endif
                </div>

                <div id="weather">
                    <h2 class="text-lg font-medium text-slate-900 dark:text-white mb-3">{{ __('flood-watch.dashboard.weather_forecast') }}</h2>
                    @if (count($weather) > 0)
                    ...
                    @else
                        <p class="p-4 rounded-lg bg-white dark:bg-slate-800 shadow-sm border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400">{{ __('flood-watch.dashboard.no_weather') }}</p>
                    @endif
                </div>

                <div id="road-status">
                    <h2 class="text-lg font-medium text-slate-900 dark:text-white mb-3">{{ __('flood-watch.dashboard.road_status') }}</h2>
                    @if (count($incidents) > 0)
                        <ul class="space-y-3">
                            @foreach ($incidents as $incident)
                                <li class="p-4 rounded-lg bg-white dark:bg-slate-800 shadow-sm border border-slate-200 dark:border-slate-700">
                                    ...
                                    @if (!empty($incident['delayTime']))
                                        <p class="text-sm text-slate-500 dark:text-slate-500 mt-1">{{ __('flood-watch.dashboard.delay') }}: {{ $incident['delayTime'] }}</p>
                                    @endif
                                </li>
                            @endforeach
                        </ul>
                    @else
                        <p class="p-4 rounded-lg bg-white dark:bg-slate-800 shadow-sm border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400">{{ __('flood-watch.dashboard.roads_clear') }}</p>
                    @endif
                </div>
                ...
                <div class="p-4 rounded-lg bg-white dark:bg-slate-800 shadow-sm border border-slate-200 dark:border-slate-700 prose prose-slate dark:prose-invert max-w-none">
                    <h2 class="text-lg font-medium text-slate-900 dark:text-white mb-2">{{ __('flood-watch.dashboard.summary') }}</h2>
                    {!! Str::markdown($assistantResponse) !!}
                </div>
            </div>
        @elseif (!$loading && !$error)
            <p class="text-slate-500 dark:text-slate-400 text-sm">{{ __('flood-watch.dashboard.prompt') }}</p>
        @endif
